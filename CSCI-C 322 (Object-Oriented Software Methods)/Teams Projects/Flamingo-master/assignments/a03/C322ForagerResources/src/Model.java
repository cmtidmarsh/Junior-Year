...
//        System.out.println("playerId " + forPlayer);
        System.out.println("players" + players);
        for (Player p: players) {
            System.out.println("iterate through player ids " + p.id);
            if (p.getID() == forPlayer) {
                int playerX = p.getX();
                int playerY = p.getY();

                System.out.println("player pos x and y: " + playerX+ ", " + playerY);

                Tile currentCell = board.get(playerX).get(playerX);
                System.out.println("cell at under player state: " + currentCell.positionX + ", " + currentCell.positionX + " score:  " + currentCell.state);
                return currentCell.state;
            }
        }
        return 0;
//        System.out.println("in gatherResource");
//
//
//
//        for(Player p: players){
//            System.out.println("On resource");
//            SCORE++; // when on resource is printed out, play picks up resource, increasing score by 1
//            if(p.getID() == forPlayer) {
//                int temp = board.get(p.positionX).get(p.positionY).state;
//                board.get(p.positionX).get(p.positionY).state = 0;
//                System.out.println("resource on tile" + temp);
//                return temp;
//
//            }
//            Tile currentcell = board.get(p.positionX).get(p.positionY);
//            currentcell.state -= 1;
//        }
//        return 0;
    }
